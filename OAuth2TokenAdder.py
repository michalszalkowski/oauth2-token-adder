...
class BurpExtender(IBurpExtender, ITab, IHttpListener):
    ...
    def	registerExtenderCallbacks(self, callbacks):
        ...
        callbacks.setExtensionName("OAuth2 Token Adder")
...
Currently supported auth flows:
    ...
    class TokenRequest:
        ...
        def go(self):
            ...
            if self.clientAuth == "Basic Auth":
                ...
                if self.grantType == "Resource Owner Password Credentials":
            ...
            try:
                token = self.findToken(jsonString, "access_token")
                if token == None:
    ...
    def checkParameters(self, grantType):
        ...
        # Client credentials
        if grantType == "Client Credentials" or grantType == "Resource Owner Password Credentials":
    ...
    def initUI(self):
        ...
        def selectGrantType(event):
            ...
            if self.cmbGrantType.getSelectedItem() == "Resource Owner Password Credentials":
                self.txtUsername.setEnabled(True)
                self.txtPassword.setEnabled(True)
                self.txtClientID.setEnabled(True)
                self.txtClientSecret.setEnabled(True)
        ...
        # Combo Boxes
        model = ('Client Credentials', 'Password Credentials', 'Resource Owner Password Credentials')
...
Source    
    - https://github.com/michalszalkowski/oauth2-token-adder""")
        ...
        self.tabbedPane.addTab("Oauth2 Token Adder", jScrollPane3)
